...
public class FeedParser {
	...
	private static class ParserHelper {
			...
			if(fullName.equals(ATOM_ENTRY)) {
				...
				if(rel == null) {
					...
					if(feedItem != null) {
						...
						if(type == null || type.equals(MIME_HTML) || type.equals(MIME_XHTML)) {
							// actually there can be multiple "alternate links"
							// this uses the LAST alternate link as the URL for the FeedItem
							feedItem.setUrl(parser.getAttributeValue("", "href"));
						}
					}
				}
			}
		...
		private Date parseDate(String datestring) throws java.text.ParseException, IndexOutOfBoundsException {
			// original version by Chad Okere (ceothrow1 at gmail dotcom) http://cokere.com/RFC3339Date.txt
